...
protected:
    ...
    void interop_task_fwd(
            ...
            std::optional<mv_acc_t> mean_acc,
            sycl::sycl_memory_storage_base_t *mean_mem,
            std::optional<mv_acc_t> var_acc,
    ...
    template <typename T>
    void maybe_init_ss(cuda_sycl_scoped_context_handler_t &sc,
            const compat::interop_handle &ih,
            nvidia::sycl_cuda_stream_t *cuda_stream, T,
        ...
        if (init_ss) {
            ...
            if (!scale_bias_mem
                ...
                cuda_stream->interop_task([&](::sycl::handler &cgh) {
                    ...
                    T a[n];
                    cudaMemcpy(a, scale_ptr, n * sizeof(T),
                ...
                cuda_stream->interop_task([&](::sycl::handler &cgh) {
                    ...
                    T a[n];
                    cudaMemcpy(
                            a, bias_ptr, n * sizeof(T), cudaMemcpyDeviceToHost);
                    cudaDeviceSynchronize();
                });
    ...
    status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return cuda_stream->interop_task([&](::sycl::handler &cgh) {
            ...
            std::optional<decltype(CTX_OUT_ACCESSOR(DNNL_ARG_WORKSPACE))>
                    wkspace_acc;
            if (!wkspace_mem->is_null()) {
                wkspace_acc = CTX_OUT_OPTIONAL_ACCESSOR(
                        DNNL_ARG_WORKSPACE, wkspace_mem);
            }
            ...
            auto nullptr_acc = CTX_OUT_OPTIONAL_ACCESSOR(DNNL_ARG_DST, nullptr);
...
struct bnorm_exec_fwd_inf_ss_t : public bnorm_exec_base_t {
    status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return cuda_stream->interop_task([&](::sycl::handler &cgh) {
            ...
            auto *scale_bias_mem = CTX_IN_MEMORY(DNNL_ARG_SCALE_SHIFT);
            auto scale_bias_acc = CTX_IN_OPTIONAL_ACCESSOR(
                    DNNL_ARG_SCALE_SHIFT, scale_bias_mem);
...
struct bnorm_exec_fwd_inf_stats_t : public bnorm_exec_base_t {
    status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return cuda_stream->interop_task([&](::sycl::handler &cgh) {
            ...
            auto *mean_mem = CTX_IN_MEMORY(DNNL_ARG_MEAN);
            auto mean_acc = CTX_IN_OPTIONAL_ACCESSOR(DNNL_ARG_MEAN, mean_mem);
            auto *var_mem = CTX_IN_MEMORY(DNNL_ARG_VARIANCE);
            auto var_acc = CTX_IN_OPTIONAL_ACCESSOR(DNNL_ARG_VARIANCE, var_mem);
...
struct bnorm_exec_fwd_t : public bnorm_exec_base_t {
    status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return cuda_stream->interop_task([&](::sycl::handler &cgh) {
            ...
            auto *mean_mem = CTX_OUT_MEMORY(DNNL_ARG_MEAN);
            auto mean_acc = CTX_OUT_OPTIONAL_ACCESSOR(DNNL_ARG_MEAN, mean_mem);
            auto *var_mem = CTX_OUT_MEMORY(DNNL_ARG_VARIANCE);
            auto var_acc
                    = CTX_OUT_OPTIONAL_ACCESSOR(DNNL_ARG_VARIANCE, var_mem);
...
struct bnorm_exec_bwd_t : public bnorm_exec_base_t {
    status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return cuda_stream->interop_task([&](::sycl::handler &cgh) {
            ...
            auto diff_src_acc = CTX_OUT_OPTIONAL_ACCESSOR(
                    DNNL_ARG_DIFF_SRC, diff_src_mem);
            auto *wkspace_mem = CTX_IN_MEMORY(DNNL_ARG_WORKSPACE);
            auto wkspace_acc
                    = CTX_IN_OPTIONAL_ACCESSOR(DNNL_ARG_WORKSPACE, wkspace_mem);
            ...
            interop_task_bwd(bnorm_impl, engine, cgh, cuda_stream, src_acc,
                    ...
                    wkspace_mem, temp_relu_output, init_ss);
        });
    }
};
    ...
    status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return cuda_stream->interop_task([&](::sycl::handler &cgh) {
            ...
            auto *diff_scale_bias_mem
                    = CTX_OUT_MEMORY(DNNL_ARG_DIFF_SCALE_SHIFT);
            auto diff_scale_bias_acc = CTX_OUT_OPTIONAL_ACCESSOR(
                    DNNL_ARG_DIFF_SCALE_SHIFT, diff_scale_bias_mem);
            auto *wkspace_mem = CTX_IN_MEMORY(DNNL_ARG_WORKSPACE);
            auto wkspace_acc
                    = CTX_IN_OPTIONAL_ACCESSOR(DNNL_ARG_WORKSPACE, wkspace_mem);
            bool init_ss = false;
