...
status_t ref_convolution_fwd_t::pd_t::init_conf() {
    ...
    conf_.single_weight_scale = attr()->scales_.get(DNNL_ARG_WEIGHTS).mask_ == 0;

    conf_.use_data_zeropoints = !attr()->zero_points_.has_default_values(DNNL_ARG_SRC_0);
    conf_.use_dst_zeropoints = !attr()->zero_points_.has_default_values(DNNL_ARG_DST);
